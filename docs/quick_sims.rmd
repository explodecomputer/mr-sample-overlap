...
gwas_sim <- function(nid = 9000, nsnp = 20, bgx = 0.08, bxy = 0.2, buy = 0.4, bux = 0.4, out_index = 1:3000)
{
	...
	# Perform MR
	d <- list()
	d$disc_all <- make_dat(disc_gwas, out_gwas)
	d$rep_all <- make_dat(rep_gwas, out_gwas)
	d$disc_sig <- make_dat(disc_gwas[index,], out_gwas[index,])
	d$rep_sig <- make_dat(rep_gwas[index,], out_gwas[index,])
...
1. We generated GWAS on 2.5k traits, with QC etc
2. Simulations: Winner's curse makes sample overlap and weak instrument bias much worse
3. Simulations: Replication and adjusting for winner's curse using UMVCUE doesn't work to solve it
4. How to use this for MR?
	- Winner's curse is an issue
	- Sample overlap is an issue
	- Weak instrument bias is an issue
	- Recommendation:
		1. external discovery; ukbb replication; ukbb outcome
		2. ukbb discovery; ukbb replication; external outcome
